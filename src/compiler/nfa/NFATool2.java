	...
	{//System.out.println("p3");
		...
		if(match("?") || match("*") || match("+")){
			...
			if(match("?")){
				int tnfa1=popNFA();
				int tnfa2=popNFA();
				addTransition(EPSILON,nfa1,tnfa1);
				addTransition(EPSILON,tnfa2,nfa2);
				addTransition(EPSILON,nfa1,nfa2);
			}
			else if(match("+")){
				int tnfa1=popNFA();
				int tnfa2=popNFA();
...
//		print stacks for checking
	...
	public static void main(String[] args) {
		NFATool2 tool = new NFATool2();
		tool.compile("(a|b)*abbs");
		tool.print_Transitions();
		tool.parse("abababbaaaaaaaabbs");
